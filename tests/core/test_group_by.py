...
def test_getitem_key(rc: Rc) -> None:
    gp = rc.group_by("count")
    assert len(gp[1]) == 6
    assert len(gp[2]) == 6


def test_getitem_key_multi(rc: Rc) -> None:
...
def test_contains(rc: Rc) -> None:
    gp = rc.group_by("count")
    assert 1 in gp
    assert 3 not in gp


def test_keys(rc: Rc) -> None:
    gp = rc.group_by("count")
    assert list(gp.keys()) == [1, 2]


def test_values(rc: Rc) -> None:
    gp = rc.group_by("count")
    assert len(list(gp.values())) == 2


def test_items(rc: Rc) -> None:
    gp = rc.group_by("count")
    assert len(list(gp.items())) == 2


def test_agg_key(rc: Rc) -> None:
